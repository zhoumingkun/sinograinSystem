...
//				create.setCellStyle(utils.Style6(workbook));
				...
				sheet.addMergedRegion(new CellRangeAddress(5+number, 5+number, (short) 5, (short) 7));
//				createCell9.setCellStyle(utils.Style6(workbook));
...
//				response.reset();
	...
	@Override
	public void expotStorageHandover(HttpServletResponse response,Handover handover){
		
		try {
			//输入模板文件
			XSSFWorkbook xssfWorkbook = new XSSFWorkbook(new FileInputStream("upload/base/带有存放位置的交接单.xlsx"));
			SXSSFWorkbook workbook = new SXSSFWorkbook(xssfWorkbook, 1000);
			POIUtils utils = new POIUtils();
			//对应Excel文件中的sheet，0代表第一个             
			Sheet sheet = xssfWorkbook.getSheetAt(0);  
